...
1. `meta(dex)` ile izlenen HIP-3 dex'lerinin (`EQUITY_DEXES`, varsayılan `xyz`) hisse
   ...
   **Kripto builder dex'leri** (`CRYPTO_DEXES`, varsayılan `para` — `para:ANSEM`):
   ...
   kv'ye (`crypto_dex_symbols`) yazılır, açılışta yüklenir. **Tabanlar ayrı:** bu
   dex'lerde fill ve pozisyon kayıt tabanı `CRYPTO_DEX_FILL_MIN_NOTIONAL` /
   `CRYPTO_DEX_MIN_POSITION_NOTIONAL` (vars. $1K) — memecoin'de $5K/$10K'lık hisse
   tabanı trader ve pozisyonların çoğunu gizliyor, liq haritasını boş gösteriyordu.
   Yalnız kayıt tabanıdır; bildirim eşikleri ($1M+) değişmez.
