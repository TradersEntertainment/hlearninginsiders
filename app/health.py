...
def limits(cfg: Config) -> dict[str, int]:
    ...
    lim = {
        ...
        "cryptoliq": int(getattr(cfg, "crypto_liq_poll_sec", 120)) * 3 + 300,
        ...
        "twaplive": int(getattr(cfg, "twap_live_eval_sec", 60)) * 5 + 300,
        "liqattack": int(getattr(cfg, "liq_attack_scan_sec", 300)) * 3 + 600,
        "sim": int(getattr(cfg, "sim_poll_sec", 60)) * 3 + 300,
        ...
        "paywatch": 60 * 5 + 600,          # ödeme izleyici: 60 sn tur (bekleyen yokken de nabız)
        "billing": 3600 * 3 + 600,         # faturalama: saatlik
        "fanout": 600,                     # dağıtım: 60 sn'de bir nabız (olay güdümlü)
        "public_digest": 2400,
        "census": 3600,                    # sayım: parti başına nabız (200 hesap); kapalıyken 10 dk'da bir
    }
...
def periods(cfg: Config) -> dict[str, int]:
    ...
    per = {
        ...
        "cryptoliq": int(getattr(cfg, "crypto_liq_poll_sec", 120)),
        ...
        "twaplive": int(getattr(cfg, "twap_live_eval_sec", 60)),
        "liqattack": int(getattr(cfg, "liq_attack_scan_sec", 300)),
        "sim": int(getattr(cfg, "sim_poll_sec", 60)),
