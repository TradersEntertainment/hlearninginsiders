...
def test_snapshot():
    async def run():
        ...
        # METİN listesi = ≥$500K ve ≤%20 (D %6'da, C $300K → listede yok);
        # GRAFİK havuzu ise ≥$200K (C dahil)
        assert [p["address"] for p in s["rows"]] == [B, A, D], "≥$500K ve ≤%20, yakından uzağa"
        assert s["n_all"] == 4 and s["n_big"] == 3 and s["mark"] == MARK["PUMP"] and s["png"]
        assert s["min_usd"] == 500_000 and s["chart_usd"] == 200_000 and s["n_chart"] == 4
        assert s["n_more"] == 0 and s["n_list_far"] == 0 and s["list_dist"] == 20.0
...
def test_near_band_beats_far_wall():
    ...
    async def run():
        ...
        assert has22(s["clusters"]), "yakın band metinde"
        # METİN mesafe sınırı %20: %20,5'teki $17.6M duvar ve %10-15'teki $6.3M metinden
        # düşer (grafikte durur, bağlam satırı söyler); ⭐ yine $2.2M
